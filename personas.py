...
[{{"occupation":"...","monthly_income":1200,"city":"...","family_status":"...","daily_routine":"...","financial_behavior":"...","apps_used":"...","work_context":"...","backstory":"...","communication_style":"...","pain_points":["...","..."],"current_solution":"...","gender":"male"}}]"""
    ...
    for attempt in range(max_attempts):
        try:
            ...
            if not _current_key:
                logger.error("[personas] Нет доступных API ключей")
                return None
            ...
            err = str(e)
            logger.warning(f"[personas] Ошибка генерации (попытка {attempt+1}/{max_attempts}): {err}")

            # Проверяем — исчерпана ли квота (суточный лимит)
            quota_keywords = (
                "quota", "exceeded", "resource_exhausted",
                "RESOURCE_EXHAUSTED", "billing", "insufficient_quota",
            )
            is_quota = any(kw.lower() in err.lower() for kw in quota_keywords)

            if is_quota:
                # Помечаем конкретный ключ как мёртвый
                if _current_key:
                    _cfg.mark_key_dead(_current_key)
                pool = _cfg.get_pool_status()
                if pool["alive_keys"] > 0 and attempt < max_attempts - 1:
                    logger.warning(
                        f"[personas] Квота ключа исчерпана — "
                        f"осталось {pool['alive_keys']} живых, пробуем следующий"
                    )
                    time.sleep(2.0)
                    continue
                else:
                    logger.error("[personas] Все ключи исчерпаны или последняя попытка")
                    return None

            if "429" in err:
                # Rate limit — штрафуем конкретный ключ
                if _current_key:
                    _cfg.rotate_key(failed_key=_current_key)
                if attempt < max_attempts - 1:
                    logger.warning(f"[personas] Rate limit 429, переключаем ключ...")
                    time.sleep(3.0)
                    continue

            # Другая ошибка — не пытаемся повторять
            return None

    logger.warning("[personas] Все попытки генерации персон исчерпаны")
    return None
